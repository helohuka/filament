...
ImGuiHelper::ImGuiHelper(Engine* engine, filament::View* view, const Path& fontPath, ImGuiContext* imGuiContext) :
    mEngine(engine), mView(view), mScene(engine->createScene()), mImGuiContext(imGuiContext ? imGuiContext : ImGui::CreateContext())
{
...
void ImGuiHelper::createAtlasTexture()
{
...
void ImGuiHelper::processImGuiCommands(ImDrawData* commands)
{
    ImGui::SetCurrentContext(mImGuiContext);
    ImGuiIO& io = ImGui::GetIO();
