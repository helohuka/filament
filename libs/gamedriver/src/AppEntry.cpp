...
struct App {
    ...
    MaterialSource materialSource = UBERSHADER; //A57 ,powervr 8xxx
...
int entry(int argc, char** argv)
{
    ...
    config.title = "GameDriver";
    config.iblDirectory = Resource::get().getRootPath() + DEFAULT_IBL;
    ...
    auto setup = [&](Engine* engine, View* view, Scene* scene)
    {
        ...
        app.viewer->setUiCallback(
            [&app, scene, view, engine]()
            {
    ...
    auto preRender = [&app](Engine* engine, View* view, Scene* scene, Renderer* renderer)
    {
        ...
        app.scene.groundMaterial->setDefaultParameter("strength", viewerOptions.groundShadowStrength);
    ...
    auto postRender = [&app](Engine* engine, View* view, Scene* scene, Renderer* renderer)
    {
        ...
        app.automationEngine->tick(GameDriver::get().getRenderEngine(), content, ImGui::GetIO().DeltaTime);
    };

    GameDriver& gd = GameDriver::get();
    gd.initConfig(config);
    gd.animate(animate);
    gd.resize(resize);
    ...
    gd.run( setup, cleanup, gui, preRender, postRender);
