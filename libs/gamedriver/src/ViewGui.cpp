...
{
    filament::math::float4& ranges = settings.view.colorGrading.ranges;
    ranges.y                       = filament::math::clamp(ranges.y, ranges.x + 1e-5f, ranges.w - 1e-5f); // darks
    ranges.z                       = filament::math::clamp(ranges.z, ranges.x + 1e-5f, ranges.w - 1e-5f); // lights

    for (size_t i = 0; i < 1024; i++)
    {
        float x             = i / 1024.0f;
        float s             = 1.0f - filament::math::smoothstep(ranges.x, ranges.y, x);
        float h             = filament::math::smoothstep(ranges.z, ranges.w, x);
        rangePlot[i]        = s;
...
inline filament::math::float3 curves(filament::math::float3 v, filament::math::float3 shadowGamma, filament::math::float3 midPoint, filament::math::float3 highlightScale)
{
    filament::math::float3 d     = 1.0f / (pow(midPoint, shadowGamma - 1.0f));
    filament::math::float3 dark  = pow(v, shadowGamma) * d;
    filament::math::float3 light = highlightScale * (v - midPoint) + midPoint;
...
static void computeCurvePlot(filament::viewer::Settings& settings, float* curvePlot)
{
    const auto& colorGradingOptions = settings.view.colorGrading;
    for (size_t i = 0; i < 1024; i++)
    {
        filament::math::float3 x{i / 1024.0f * 2.0f};
        filament::math::float3 y = curves(x,
...
static void computeToneMapPlot(filament::viewer::ColorGradingSettings& settings, float* plot)
{
    float                 hdrMax = 10.0f;
    filament::ToneMapper* mapper;
    switch (settings.toneMapping)
    {
        case filament::viewer::ToneMapping::LINEAR:
            mapper = new filament::LinearToneMapper;
            break;
        case filament::viewer::ToneMapping::ACES_LEGACY:
            mapper = new filament::ACESLegacyToneMapper;
            break;
        case filament::viewer::ToneMapping::ACES:
            mapper = new filament::ACESToneMapper;
            break;
        case filament::viewer::ToneMapping::FILMIC:
            mapper = new filament::FilmicToneMapper;
            break;
        case filament::viewer::ToneMapping::GENERIC:
            mapper = new filament::GenericToneMapper(
            ...
            break;
        case filament::viewer::ToneMapping::DISPLAY_RANGE:
            mapper = new filament::DisplayRangeToneMapper;
            break;
    }
...
static void colorGradingUI(filament::viewer::Settings& settings, float* rangePlot, float* curvePlot, float* toneMapPlot)
{
...
void GameDriver::sceneSelectionUI()
{
