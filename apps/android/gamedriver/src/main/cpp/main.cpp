...
int SDL_main(int argc, char* argv[]) {
    ...
    //JavaVM* vm = (JavaVM*)SDL_AndroidGetJNIEnv();
    ...
    auto setup = [&app](Engine* engine, View* view, Scene* scene) {
        app.skybox = Skybox::Builder().build(*engine);
    ...
    FilamentApp::get().animate([&app](Engine* engine, View* view, double now) {
        ...
        void* verts = malloc(48);
        TRIANGLE_VERTICES[0].position.y = sin(now * 4);
        memcpy(verts, TRIANGLE_VERTICES, 48);
        vb->setBufferAt(*engine, 0, VertexBuffer::BufferDescriptor(verts, 48,
